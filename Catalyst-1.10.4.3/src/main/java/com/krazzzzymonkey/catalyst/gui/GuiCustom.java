...
import com.krazzzzymonkey.catalyst.configuration.elements.*;
...
public class GuiCustom extends GuiScreen implements GuiYesNoCallback
{
	...
	private void loadSplashTexts()
	{
		if (guiConfig.splashText != null)
		{
			...
			this.splashText = seperateLines[(rand.nextInt(seperateLines.length))];
		}
	}
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks)
	{
		...
		if (panorama != null)
		{
			if (guiConfig.panorama != null)
			{
				...
				{
					GuiCustom mainMenu = Main.config.getGUI("mainmenu");
		...
		{
			this.buttonList.get(w).drawButton(this.mc, mouseX, mouseY, partialTicks);
		}
		...
		{
			this.labelList.get(w).drawLabel(this.mc, mouseX, mouseY);
		}
	}
